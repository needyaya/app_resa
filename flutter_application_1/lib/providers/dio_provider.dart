...
class DioProvider {
  ...
  Future<dynamic> bookAppointment(
      String date, String day, String time, int room, String token) async {
    try {
      var response = await Dio().post('http://127.0.0.1:8000/api/book',
          data: {'date': date, 'day': day, 'time': time, 'room_id': room},
          options: Options(headers: {'Authorization': 'Bearer $token'}));
  ...
  Future<dynamic> storeReviews(
      String reviews, double ratings, int id, int room, String token) async {
    try {
